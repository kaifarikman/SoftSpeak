...
def load_model() -> SentenceTransformer:
    ...
    try:
        ...
        if cache_dir.exists():
            ...
            if incomplete_files:
                logger.info(
                    f"Найдено {len(incomplete_files)} неполных загрузок, очищаем..."
                )
        ...
        logger.info(
            "Модель весит ~500MB, это может занять 5-15 минут в зависимости от скорости интернета"
        )
        ...
        logger.info(
            f"Размер модели в памяти: ~{_model_st.get_sentence_embedding_dimension()} измерений"
        )
        return _model_st
