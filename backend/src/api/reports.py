...
class CreateReportRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=500, description="Дополнительное описание (для 'другое')"
    )
...
@router.post(
    "", response_model=ReportCreateResponse, status_code=status.HTTP_201_CREATED
)
async def create_report(
    request: CreateReportRequest, email: str, session: AsyncSession = Depends(get_db)
...
async def cancel_report(
    report_id: int, email: str, session: AsyncSession = Depends(get_db)
) -> dict:
    user = await get_user_by_email(session, email)
    if not user:
        raise HTTPException(
    ...
    return {"success": True, "message": "Жалоба отменена. Чат разблокирован."}
