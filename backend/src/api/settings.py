...
async def get_public_profile_by_nickname(
    nickname: str, session: AsyncSession = Depends(get_db)
...
async def update_nickname_endpoint(
    email: str, request: UpdateNicknameRequest, session: AsyncSession = Depends(get_db)
) -> SettingsResponse:
    ...
    if not success:
        return SettingsResponse(
            success=False, message=error_message or "Ошибка обновления никнейма"
        )
    chat_data = await get_chat_data_for_user(session, email)
    return SettingsResponse(
        success=True, message="Никнейм изменен", chat_data=chat_data.model_dump()
    )


@router.put("/profile/bio/{email}", response_model=SettingsResponse)
async def update_bio_endpoint(
    email: str, request: UpdateBioRequest, session: AsyncSession = Depends(get_db)
) -> SettingsResponse:
    await verify_user_active(email, session)
    user = await get_user_by_email(session, email)
...
async def update_notification_settings_endpoint(
    ...
    if not updated_user:
        return SettingsResponse(success=False, message="Ошибка обновления настроек")
    return SettingsResponse(success=True, message="Настройки уведомлений обновлены")


@router.put("/account/password/{email}", response_model=SettingsResponse)
async def change_password_endpoint(
    email: str, request: ChangePasswordRequest, session: AsyncSession = Depends(get_db)
) -> SettingsResponse:
    await verify_user_active(email, session)
    user = await get_user_by_email(session, email)
    if request.new_password != request.new_password_confirm:
        return SettingsResponse(
            success=False, message="Неправильный пароль или новые пароли не совпадают"
        )
    success, error_message = await settings_crud.change_password(
        session, user.id, request.old_password, request.new_password
    )
    if not success:
        return SettingsResponse(
            success=False, message=error_message or "Ошибка изменения пароля"
        )
    return SettingsResponse(success=True, message="Пароль изменен")


@router.get("/status/{email}")
async def get_user_status(email: str, session: AsyncSession = Depends(get_db)):
    user = await get_user_by_email(session, email)
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Пользователь не найден"
