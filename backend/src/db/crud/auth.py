...
async def issue_email_verification_code(
    session: AsyncSession, *, nickname: str, email: str, raw_password: str
) -> Tuple[User, EmailVerificationCode]:
    import re

    nickname = nickname.strip()
    if not re.match("^[a-zA-Zа-яА-ЯёЁ0-9_-]+$", nickname):
        raise ValueError(
    ...
    if user:
        if user.is_active:
            raise ValueError(
                f"Пользователь с никнеймом '{nickname}' уже существует и подтвержден. Пожалуйста, войдите в систему."
            )
        if user.is_banned:
            raise ValueError(
                "Ваш аккаунт заблокирован администратором. Доступ запрещен."
            )
        cooldown_seconds = 60
        ...
        if last_code:
            code_created_at = last_code.expires_at - timedelta(
                minutes=settings.verification_code_ttl_min
            )
            if (now - code_created_at).total_seconds() < cooldown_seconds:
                raise ValueError(
                    f"Подождите {cooldown_seconds} секунд перед повторным запросом кода"
                )
...
async def confirm_email_verification_code(
    session: AsyncSession, *, nickname: str, code: str
) -> Optional[User]:
    user = await get_user_by_nickname(session, nickname)
    if not user:
        return None
    if user.is_banned:
        return None
    now = datetime.now(timezone.utc)
    stmt: Select[EmailVerificationCode] = (
        select(EmailVerificationCode)
        .where(
            and_(
                EmailVerificationCode.user_id == user.id,
