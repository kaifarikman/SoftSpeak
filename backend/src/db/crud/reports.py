...
async def get_user_reports(session: AsyncSession, user_id: int) -> list[Report]:
    stmt = (
        select(Report)
...
async def ban_user_from_report(
    session: AsyncSession, report_id: int, admin_id: int
) -> Tuple[bool, Optional[str]]:
    report_stmt = select(Report).where(Report.id == report_id)
    report_result = await session.execute(report_stmt)
    report = report_result.scalar_one_or_none()
...
async def unban_user(
    session: AsyncSession, user_id: int, admin_id: int
) -> Tuple[bool, Optional[str]]:
