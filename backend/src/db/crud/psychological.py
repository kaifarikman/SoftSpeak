"""CRUD-операции для работы с психологическим профилем."""
...
async def get_questions_by_category(
    ...
    """Получает вопросы категории, отсортированные по порядку."""
...
async def get_next_question_for_user(
    ...
    Возвращает (question, current_number, total_count) или None если все вопросы отвечены.
    Упрощенная версия: просто берем вопросы по порядку категорий.
    """
    # Получаем все категории, отсортированные по order
    ...
    # Всего должно быть 10 вопросов (по одному на категорию)
    total_questions = 10
    
    # Ищем первый неотвеченный вопрос по порядку категорий
    for category in categories:
        # Получаем первый активный вопрос из категории
        questions = await get_questions_by_category(session, category.id, only_active=True)
        if questions:
            # Берем первый вопрос из категории
...
async def get_user_answers_count(session: AsyncSession, user_id: int) -> int:
    """Получает количество ответов пользователя."""
...
async def create_psychological_profile(
    ...
    """Создает психологический портрет пользователя."""
...
async def has_completed_profile(session: AsyncSession, user_id: int) -> bool:
    """Проверяет, завершил ли пользователь создание профиля."""
