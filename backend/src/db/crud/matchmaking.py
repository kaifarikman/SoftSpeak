...
async def join_matchmaking_queue(
    ...
    if queue_entry:
        ...
        queue_entry = MatchmakingQueue(user_id=user_id, is_searching=True)
        session.add(queue_entry)
    await session.commit()
    await session.refresh(queue_entry)
    logger.info(
        f"join_matchmaking_queue: Пользователь {user_id} успешно добавлен/обновлен в очереди"
    )
    return queue_entry


async def leave_matchmaking_queue(session: AsyncSession, user_id: int) -> bool:
    stmt = select(MatchmakingQueue).where(MatchmakingQueue.user_id == user_id)
    result = await session.execute(stmt)
    queue_entry = result.scalar_one_or_none()
    if queue_entry:
        ...
        await session.commit()
        logger.info(
            f"leave_matchmaking_queue: Пользователь {user_id} удален из очереди"
        )
        return True
...
async def get_matchmaking_queue_count(
    session: AsyncSession, exclude_user_id: Optional[int] = None
) -> int:
...
async def find_match(
    session: AsyncSession, user_id: int, threshold: float = None
    ...
    if not user.psychological_profile:
        logger.warning(
            f"find_match: У пользователя {user_id} нет психологического профиля"
        )
        return None
    ...
    if not other_users:
        logger.info(
            f"find_match: Нет подходящих пользователей с профилями для {user_id}"
        )
        return None
    logger.info(
    ...
    if not best_match_id:
        ...
        return None
    logger.info(
        f"find_match: Найден лучший матч для {user_id}: пользователь {best_match_id}"
    )
    ...
    if existing_chat:
        ...
        stmt2 = select(MatchmakingQueue).where(
            MatchmakingQueue.user_id == best_match_id
        )
    ...
    user1_id_sorted, user2_id_sorted = (
        (user_id, best_match_id)
        if user_id < best_match_id
        else (best_match_id, user_id)
    )
    check_stmt = (
    ...
    while user1_alias == user2_alias and attempts < max_attempts:
        logger.warning(
            f"find_match: Алиасы совпали ({user1_alias}), перегенерируем user2_alias"
        )
        user2_alias = await random_names.generate_random_alias(session)
        attempts += 1
    if user1_alias == user2_alias:
        logger.error(
            f"find_match: Не удалось сгенерировать уникальные алиасы после {max_attempts} попыток"
        )
    ...
    logger.info(
        f"find_match: Успешно создан новый чат {chat.id} между {user_id} и {best_match_id}"
    )
    return chat
...
async def create_anonymous_message(
    session: AsyncSession, chat_id: int, sender_id: int, content: str | None = None
) -> AnonymousMessage:
    ...
    message = AnonymousMessage(
        chat_id=chat_id, sender_id=sender_id, content=content or ""
    )
    session.add(message)
...
async def mark_messages_as_read(
    ...
    if not chat:
        raise ValueError("Чат не найден")
    unread_messages = [
        msg for msg in chat.messages if msg.sender_id != user_id and (not msg.is_read)
    ]
    count = 0
    ...
    if count > 0:
        await session.commit()
        logger.info(
            f"mark_messages_as_read: Помечено {count} сообщений как прочитанных в чате {chat_id} для пользователя {user_id}"
        )
    return count
