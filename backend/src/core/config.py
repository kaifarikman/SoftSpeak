...
class Settings(BaseSettings):
    ...
    def validate_jwt_secret(cls, v: str) -> str:
        if len(v) < 32:
            raise ValueError(
                f"JWT_SECRET должен быть минимум 32 символа. Текущая длина: {len(v)}. Это критично для безопасности!"
            )
        unsafe_values = [
            "change_me",
        ...
        if v_lower in [uv.lower() for uv in unsafe_values]:
            ...
            if dev_mode:
                ...
                logger.warning(
                    f"⚠️  ВНИМАНИЕ: Используется небезопасный JWT_SECRET в dev-режиме: '{v}'. ОБЯЗАТЕЛЬНО измените на уникальный ключ в продакшене!"
                )
                return v
            else:
                raise ValueError(
                    f"JWT_SECRET не может быть небезопасным значением. ОБЯЗАТЕЛЬНО установите уникальный секретный ключ!"
                )
        return v
